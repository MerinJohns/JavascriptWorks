...
// var total_films=filims.length
// console.log((total_films));

//movies whose runtime below 140

// var runtime_filter=filims.filter(f=>f.runtime < 140).map(f=>f.title)
// console.log(runtime_filter);

//movies with highest runtime

// var highest_runtime=filims.reduce((f1,f2)=>Number(f1.runtime) > Number(f2.runtime) ? f1 : f2)
// console.log(highest_runtime);

//movies with lowest runtime

// var lowest_runtime=filims.reduce((f1,f2)=>Number(f1.runtime) < Number(f2.runtime) ? f1.title : f2.title)
// console.log(lowest_runtime);
...
// filims.map(f=> f.year in wc ? wc[f.year]+=1 : wc[f.year]=1)

//console.table(wc)

//action movie names

// var action_movies=filims.filter(f=>f.genres.includes("Action")).map(f=>f.title)
// console.log(action_movies);

//movie with genre either  action or drama

// var movies_filter=filims.filter(f=>f.genres.includes("Action" || f.genres=="Drama")).map(f=>f.title)
// console.log(movies_filter);
